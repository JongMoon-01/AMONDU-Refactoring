...
from app.database import get_db, Session as DBSession, Score
...
def create_score(score: ScoreCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db_score = Score(
            user_id=session.user_id,  # 세션에서 user_id 가져옴
...
def get_user_analytics(user_id: int, days: int = 30, db: Session = Depends(get_db)):
    """사용자 분석 데이터 조회 (user_id는 edtech userId)"""
